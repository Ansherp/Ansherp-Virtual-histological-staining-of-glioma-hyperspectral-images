...
def define_G(input_nc, output_nc, ngf, netG, n_downsample_global=3, n_blocks_global=9, n_local_enhancers=1,
    ...
    if netG == 'global':
        ...
        netG = Trans_UNet(input_nc, output_nc, conv_nd)

    elif netG == 'UNet_Usual':
        netG = UNet_Usual(input_nc, output_nc)
    elif netG == 'UNet_Usual3D':
        netG = UNet_Usual_3D(input_nc, output_nc)
...
class UNet_Usual(nn.Module):
    def __init__(self, in_channels=3, out_channels=3):
        super(UNet_Usual, self).__init__()
        self.init_layer = nn.Sequential(nn.Conv2d(in_channels, 64, 3, 1, 1),
                                        nn.InstanceNorm2d(32),
                                        nn.LeakyReLU(0.2),
                                        )

        self.down1 = UNet_Down(64, 128)  # 3d_C = 6
        self.down2 = UNet_Down(128, 256)  # 3d_C = 6
        self.down3 = UNet_Down(256, 512)  # 3d_C = 6
        self.down4 = UNet_Down(512, 1024)  # 3d_C = 6

        self.Res = Unet_ResBlock(1024, 4)

        self.up1 = UNet_Up(1024, 512)
        self.up2 = UNet_Up(1024, 256)
        self.up3 = UNet_Up(512, 128)
        self.up4 = UNet_Up(256, 64)
...
#           U-NET 3D
##############################
class UNet_Down_3D(nn.Module):
    def __init__(self, in_size, out_size, dropout=0.0):
        super(UNet_Down_3D, self).__init__()
        self.model1 = nn.Sequential(nn.Conv3d(in_size, out_size, 3, 2, 1, bias=False),
                                    nn.InstanceNorm3d(out_size),
                                    nn.LeakyReLU(0.2), )
        layers = [
            nn.Conv3d(out_size, out_size, 3, 1, 1, bias=False),
            nn.InstanceNorm3d(out_size),
            nn.LeakyReLU(0.2),
            nn.Conv3d(out_size, out_size, 3, 1, 1, bias=False),
            nn.InstanceNorm3d(out_size),
...
class Unet_ResBlock_3D(nn.Module):
    """Define a Resnet block"""

    def __init__(self, dim, n):
        super(Unet_ResBlock_3D, self).__init__()
        conv_block = []
        for i in range(n):
            conv_block += [nn.Conv3d(dim, dim, kernel_size=3, padding=1), nn.InstanceNorm3d(dim), nn.ReLU()]
...
class UNet_Up_3D(nn.Module):
    def __init__(self, in_size, out_size, dropout=0.0):
        super(UNet_Up_3D, self).__init__()

        layers = [
            nn.ConvTranspose3d(in_size, out_size, (3,4,4), 2, 1, bias=False),
            nn.InstanceNorm3d(out_size),
            nn.ReLU(inplace=True),
            nn.ConvTranspose3d(out_size, out_size, 3, 1, 1, bias=False),
            nn.InstanceNorm3d(out_size),
            nn.ReLU(inplace=True),
            nn.ConvTranspose3d(out_size, out_size, 3, 1, 1, bias=False),
            nn.InstanceNorm3d(out_size),
...
class UNet_Usual_3D(nn.Module):
    def __init__(self, in_channels=3, out_channels=3):
        super(UNet_Usual_3D, self).__init__()
        self.init_layer = nn.Sequential(nn.Conv3d(in_channels, 64, 3, 1, 1),
                                        nn.InstanceNorm3d(32),
                                        nn.LeakyReLU(0.2),
                                        )

        self.down1 = UNet_Down_3D(64, 128)  # 3d_C = 6
        self.down2 = UNet_Down_3D(128, 256)  # 3d_C = 6
        self.down3 = UNet_Down_3D(256, 512)  # 3d_C = 6
        self.down4 = UNet_Down_3D(512, 1024)  # 3d_C = 6

        self.Res = Unet_ResBlock_3D(1024, 4)

        self.up1 = UNet_Up_3D(1024, 512)
        self.up2 = UNet_Up_3D(1024, 256)
        self.up3 = UNet_Up_3D(512, 128)
        self.up4 = UNet_Up_3D(256, 64)
...
if __name__ == "__main__":
    netG = UNet_Usual_3D(7, 3)
    print('gobal net load')
    print(netG)
    input = torch.ones([1, 7, 1, 128, 128])
    out = netG(input)
    print(out.shape)
    pass