...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        loss_dict = dict(zip(model.module.loss_names, losses)) if len(opt.gpu_ids) != 0 else dict(zip(model.loss_names, losses))
        # calculate final loss scalar
        loss_D = (loss_dict['D_fake'] + loss_dict['D_real']) * 0.5
        loss_G = loss_dict['G_GAN'] + loss_dict.get('G_GAN_Feat',0) + loss_dict.get('G_VGG',0) + loss_dict.get('G_HE',0)
